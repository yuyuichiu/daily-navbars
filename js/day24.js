...
// Pig Latin
function pigLatin(){
    var str = document.getElementById("pigInput").value;
    var pigOutput = document.getElementById("pigOutput");

    var inputArr = str.split(" ");
    var result = "";
    for (i = 0; i < inputArr.length; i++){
        wordArr = inputArr[i].split("");
        for (le = 1; le < wordArr.length; le++){ result += wordArr[le]; }
        // Skip if first letter is a punctuation mark & it is alone
        if (/[.!?:;_]/.test(wordArr[0]) && inputArr[i].length == 1){ result += wordArr[0]; }
        else { result += (wordArr[0] + "ay ") };
    }

    pigOutput.innerText = result.trim();
}